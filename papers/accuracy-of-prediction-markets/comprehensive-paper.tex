...
\begin{tikzpicture}
    \begin{axis}[
        ...
        ylabel={MAD (\%)},
        title={Liquidity Threshold Analysis: MAE by Trade Count},
        ybar,
        bar width=25pt,
    ...
    \addplot[fill=kalshiblue] coordinates {
        (1--10,5.91) (11--50,4.02) (51--200,3.19) (201--1K,0.39) (1K--5K,0.50) (5K--25K,1.67) (25K--100K,5.74) (100K+,16.32)
    };
    
    \end{axis}
\end{tikzpicture}
\caption{\textbf{Liquidity Threshold Analysis.} Calibration error (MAD) by trade count bucket. Sharp decline occurs around 200 trades (transition from 51--200 to 201--1K bucket), representing the critical threshold where markets transition from thin to liquid regimes. Red dashed line marks approximately 200 trades.}
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        ylabel={MAE (\%)},
        title={Volume Effect: 4.2× Improvement from Ultra-Thin to Very Liquid},
        ybar,
        bar width=30pt,
    ...
    \addplot[fill=errorred] coordinates {
        (Ultra-Thin,16.37) (Thin,12.84) (Medium,9.13) (Liquid,7.29) (Very Liquid,3.87)
    };
    
    \end{axis}
\end{tikzpicture}
\caption{\textbf{Volume Effect on Calibration.} MAE declines monotonically with volume, showing a 4.2-fold improvement from ultra-thin to very liquid markets. The pattern is consistent with Ottaviani--S{\o}rensen's prediction that volume accumulation improves information aggregation.}
\label{fig:volume-analysis}
\end{figure}

Figure~\ref{fig:volume-analysis} presents our volume effect analysis, visualizing the dramatic 4.2$\times$ improvement.
...
\begin{tikzpicture}
    \begin{axis}[
        width=0.9\textwidth,
        height=7cm,
        xlabel={Market Age Cohort},
        ylabel={MAE (\%)},
        title={Age Effect: 5.4× Improvement from Very-Short to Very-Long Markets},
        ybar,
        bar width=32pt,
        symbolic x coords={V.Short, Short, Medium, Long, V.Long},
        xtick=data,
        x tick label style={font=\small},
        ymin=0, ymax=25,
        nodes near coords,
    ...
    \end{axis}
\end{tikzpicture}
\caption{\textbf{Market Age Effect.} Mean MAE by market age cohort with 95\% confidence intervals. The 5.4-fold improvement from very-short to very-long markets is highly statistically significant, though partially confounded with volume accumulation (older markets have more trades). Error bars widen for very-long cohort due to smaller sample size (N=361).}
\label{fig:age-analysis}
\end{figure}

Figure~\ref{fig:age-analysis} presents mean MAE by age cohort with confidence intervals, showing the dramatic 5.4-fold improvement.
